...
def prepare_train_data (df_in, period):
    ...
    df_in = df_in.drop(['last_cycle'], axis=1)
...
def prepare_test_data(df_test_in, df_truth_in, period):
    ...
    df_test_in = df_test_in.drop(['last_cycle'], axis=1)
...
hOut = helper.openHelper()
helper.main_title(hOut,"ETL - Creacion de los Datos")
...
s = df_train_raw.head()
helper.title_text(hOut,"Datos en Crudo",str(s))

#drop extra space columnn
df_train_raw = df_train_raw.drop([26,27], axis=1)

#assign column names
df_train_raw.columns = col_names
s = df_train_raw.head()
helper.title_text(hOut,"Datos de Cabecera",str(s))

# get some stat
s = df_train_raw.describe()
helper.title_text(hOut,"Estadisticas de Datos",str(s))
...
#drop extra space columnn
df_test_raw = df_test_raw.drop([26,27], axis=1)
